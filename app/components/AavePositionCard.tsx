...
export function AavePositionCard({ assets, type }: AavePositionCardProps) {
	...
	const formatNum = (stringedNum: string, decimals: number) => {
		return parseFloat(formatEther(stringedNum as unknown as bigint)).toFixed(decimals)
	}

	const formatAmount = (num: number, decimals: number) => {
		// If the number is very large (likely in wei/1e18 format), convert it
		if (num > 1e15) {
		...
		<div className={`${bgColor} border ${borderColor} rounded-lg p-6 shadow-md`}>
			<div className="flex justify-between items-center mb-4">
				...
				<div className="text-right">
					...
					<div className="text-2xl font-bold">${formatNum(totalValue.toString(), 2)}</div>
				</div>
			</div>
							...
							<div className="flex items-center justify-between">
								...
								<div className="text-right">
									<div className="font-bold text-lg">
										{formatAmount(amount, 4)} {asset.symbol}
									</div>
									<div className="text-sm text-gray-600">
										${formatNum(asset.value.toString(), 2)}
									</div>
									<div className="text-xs text-gray-500">
										@ ${formatNum(asset.price.toString(), 2)}
									</div>
								</div>
							</div>
						</div>
